...
def _strategy_bars(
    *,
    ...
    idx = pd.date_range("2020-01-01", periods=len(efa_returns), freq="B")
    return make_panel(
        {
            "SPY": _price_series(idx, spy_returns, 100.0),
            "QQQ": _price_series(idx, qqq_returns, 110.0),
            "IWM": _price_series(idx, iwm_returns, 90.0),
            "EFA": _price_series(idx, efa_returns, 95.0),
            "BIL": _price_series(idx, bil_returns, 100.0),
        }
    )


def test_dual_mom_vol10_cash_selects_risk_asset_when_it_beats_bil() -> None:
    periods = 260
    idx = np.arange(periods)
    bars = _strategy_bars(
        efa_returns=np.where(idx % 2 == 0, 0.004, -0.001),
    ...
    efa_active = weights["EFA"] > 0.0
    assert bool(efa_active.any())
    assert bool((weights.loc[efa_active, "EFA"] > 0.0).all())
    assert bool(np.isclose(weights.loc[efa_active, "BIL"], 0.0, atol=1e-12).all())


def test_dual_mom_vol10_cash_falls_back_to_bil_when_absolute_momentum_fails() -> None:
    periods = 260
    bars = _strategy_bars(
...
def test_dual_mom_vol10_cash_reduces_weight_when_realized_vol_is_high() -> None:
    ...
    efa_active = weights["EFA"] > 0.0
    assert bool(efa_active.any())
    assert float(weights.loc[efa_active, "EFA"].max()) < 1.0
    assert float(weights.loc[efa_active, "EFA"].min()) > 0.0
...
def test_dual_mom_vol10_cash_cli_next_action_json_smoke_one_line(tmp_path: Path) -> None:
    repo_root, env = _repo_root_and_env()
    periods = 320
    idx = np.arange(periods)
    store = LocalStore(base_dir=tmp_path)
    close_map = {
        "SPY": _price_series(pd.date_range("2020-01-01", periods=periods, freq="B"), np.full(periods, 0.0005), 100.0),
        "QQQ": _price_series(pd.date_range("2020-01-01", periods=periods, freq="B"), np.where(idx % 2 == 0, 0.0015, -0.0010), 105.0),
        "IWM": _price_series(pd.date_range("2020-01-01", periods=periods, freq="B"), np.full(periods, -0.0001), 95.0),
        "EFA": _price_series(pd.date_range("2020-01-01", periods=periods, freq="B"), np.where(idx % 2 == 0, 0.03, -0.01), 98.0),
        "BIL": _price_series(pd.date_range("2020-01-01", periods=periods, freq="B"), np.full(periods, 0.0001), 100.0),
    }
    for symbol, close in close_map.items():
    ...
    cmd = [
        ...
        "0.10",
        "--start",
        close_map["SPY"].index[0].date().isoformat(),
        "--end",
        close_map["SPY"].index[-1].date().isoformat(),
        "--no-plot",
        "--next-action-json",
        "--data-dir",
        str(tmp_path),
    ...
    assert "dual_mom_vol10_cash" in str(payload["event_id"])
