"""Volatility-capped dual momentum sleeve with defensive cash-like fallback."""
...
class DualMomentumVol10CashStrategy(Strategy):
    """Select the strongest risk asset and size it by realized volatility."""

    # For liquid risk ETFs, sub-1% annualized realized vol is more likely to reflect
    # flatlined/stale data than a trustworthy sizing input. Treat those readings as
    # unusable and stay defensive instead of silently promoting a full-size sleeve.
    _MIN_USABLE_ANNUALIZED_VOL = 0.01
    ...
    def __init__(
        ...
        if not self.risk_symbols:
            raise ValueError(
                "symbols must include at least one risk asset distinct from defensive_symbol."
            )
    ...
    def generate_signals(self, bars: pd.DataFrame) -> pd.DataFrame:
        ...
        realized_vol = (
            close.pct_change()
            .rolling(window=self.vol_lookback, min_periods=self.vol_lookback)
            .std(ddof=1)
            * math.sqrt(float(self.annualization))
        )
        decision_mask = self._decision_rebalance_mask(bars.index)

        weights = pd.DataFrame(0.0, index=bars.index, columns=universe, dtype=float)
        ...
        for i, (dt, is_decision) in enumerate(decision_mask.items()):
            ...
            # Default to defensive on every decision date. That keeps both warm-up
            # periods and unusable volatility estimates deterministic and conservative.
            selected_symbol = self.defensive_symbol
            target_weight = 1.0

            if pd.notna(defensive_momentum) and not risk_row.empty:
                ...
                if best_momentum > float(defensive_momentum):
                    ...
                    if (
                        math.isfinite(asset_vol_value)
                        and asset_vol_value > self._MIN_USABLE_ANNUALIZED_VOL
                    ):
                        selected_symbol = best_symbol
                        target_weight = min(1.0, self.target_vol / asset_vol_value)

            weights.loc[next_dt, selected_symbol] = float(target_weight)
