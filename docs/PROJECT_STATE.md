...
- Current promoted SHA: `2f4de85a18ea813dc44e7e17b54c682dab70cd54` (`origin/master` and runtime SHA represented by this checkpoint)
...
- Current objective: stay in Stage 2 forward-evidence accumulation / hold for the approved IBKR PaperTrader lane for `primary_live_candidate_v1`; keep the reopened bounded Stage 2 shadow daily-ops lane armed to the explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed` so the installed scheduler refreshes retained evidence automatically in the local-only shadow lane while the official `current_decision` remains manual and any future shadow target remains opt-in by later explicit control-plane opening/configuration
...
- Runtime / lane status:
  ...
  - the bounded Stage 2 shadow daily-ops lane now exists in promoted repo truth as the reusable local-only, shadow-only recurring evidence workflow surface for explicitly opened/configured Stage 2 shadow targets; the tracked config is currently armed to that same reopened pair, so scheduled runs now refresh retained comparison artifacts daily instead of defaulting to an explicit retained no-op
  ...
  - a repo-managed Windows daily EOD scheduler install surface now exists for that same bounded shadow runner, and the installed scheduler can now execute the already-open local-only refresh lane without editing docs, auto-arming a different target, or broadening the approved IBKR PaperTrader lane
...
- bounded shadow work is approved in parallel, but it must stay local-only for shadow strategies; the currently reopened shadow daily-ops pair is armed only for retained-evidence refresh, future Stage 2 shadow automation remains opt-in by later explicit control-plane opening/configuration, all control-plane decisions remain manual, and the scheduler surface must not broaden the Stage 2 IBKR PaperTrader lane or auto-open Stage 3
...
- do not open a new shadow build slice or add another shadow pair from this checkpoint refresh; continue primary-lane forward-evidence accumulation / hold while keeping the current local-only shadow target bounded and outside the approved IBKR PaperTrader lane unless a later manual control-plane update explicitly records and justifies another target
...
- Current explicitly opened shadow pair in repo live state: `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, reopened for automated retained-evidence refresh only.
