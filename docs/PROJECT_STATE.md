...
- Last completed milestone: the promoted Stage 2 shadow comparison flow slice completed the shadow-only primary-vs-shadow comparison/reporting layer, robustness harness, and shadow review scoreboard for the explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, so the next bounded repo step is to run the retained comparison package and then the shadow decision gate without broadening the approved IBKR PaperTrader lane
- Runtime / lane status:
...
- next bounded shadow-only repo work is to run the retained primary-vs-shadow backtest/walk-forward comparison package for the explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, then optional local-only replay if it adds value, then the shadow decision gate, while keeping shadow strategies local-only and without broadening the approved IBKR PaperTrader lane or opening Stage 3
...
- Priority 1: run the retained primary-vs-shadow backtest/walk-forward comparison package for the explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed` using the already-promoted comparison/reporting, robustness, and shadow review scoreboard flow.
- Priority 2: optional local-only replay if it adds value, then the shadow decision gate while keeping shadow strategies local-only during Stage 2 and outside the approved IBKR PaperTrader lane.
