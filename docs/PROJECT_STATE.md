...
- Runtime / lane status:
  ...
  - the bounded Stage 2 shadow daily-ops lane now exists in promoted repo truth as a local-only, shadow-only runner bounded to that reopened pair; the tracked config is armed to that same pair, so scheduled runs now refresh retained comparison artifacts daily instead of defaulting to an explicit retained no-op
  ...
  - a repo-managed Windows daily EOD scheduler install surface now exists for that same bounded shadow runner, and the installed scheduler can now execute that already-open local-only refresh lane without editing docs, auto-arming any different target, or broadening the approved IBKR PaperTrader lane
...
- bounded shadow work is approved in parallel, but it must stay local-only for shadow strategies; the currently reopened shadow daily-ops pair is armed only for retained-evidence refresh, any broader Stage 2 shadow-runner reuse would require a later manual control-plane + repo slice, all control-plane decisions remain manual, and the scheduler surface must not broaden the Stage 2 IBKR PaperTrader lane or auto-open Stage 3
