...
- Current promoted SHA: `0012adfdbe49c67bdec180030eb9bc3c2dfe9d67` (`origin/master` and runtime SHA represented by this checkpoint)
...
- Current objective: stay in Stage 2 forward-evidence accumulation / hold for the approved IBKR PaperTrader lane for `primary_live_candidate_v1`; reopen the bounded Stage 2 shadow daily-ops lane for the sole explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed` so the installed scheduler refreshes retained evidence automatically in the local-only shadow lane while the official `current_decision` remains manual
- Last completed milestone: the bounded Stage 2 shadow daily-ops control-plane slice is now reopened in repo truth for the explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, and the tracked runner config is intentionally armed so the installed scheduler refreshes retained evidence instead of producing an explicit retained no-op
- Runtime / lane status:
  ...
  - the bounded Stage 2 shadow daily-ops lane now exists in promoted repo truth as a local-only, shadow-only runner with tracked config explicitly armed to that same bounded pair; scheduled runs now refresh retained comparison artifacts daily instead of defaulting to an explicit retained no-op
  ...
  - a repo-managed Windows daily EOD scheduler install surface now exists for that same bounded shadow runner, and the installed scheduler can now execute the already-open local-only refresh lane without editing docs, auto-arming a different pair, or broadening the approved IBKR PaperTrader lane
...
- bounded shadow work is approved in parallel, but it must stay local-only for shadow strategies; the reopened shadow daily-ops pair is armed only for retained-evidence refresh, all control-plane decisions remain manual, and the scheduler surface must not broaden the Stage 2 IBKR PaperTrader lane or auto-open Stage 3
...
- keep the bounded Stage 2 shadow daily-ops lane running only for the reopened explicit pair `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, and review the refreshed retained artifacts manually without treating automation as a decision-gate change
- do not open a new shadow build slice or add another shadow pair from this checkpoint refresh; continue primary-lane forward-evidence accumulation / hold while keeping this single reopened local-only shadow pair bounded and outside the approved IBKR PaperTrader lane

### Approved Shadow-Work Queue

- Only one active next shadow candidate is approved unless evidence clearly justifies more.
- Sole active next shadow pair in repo live state: `primary_live_candidate_v1` vs `primary_live_candidate_v1_vol_managed`, reopened for automated retained-evidence refresh only.
...
- Any future shadow candidate or additional bounded shadow slice must be re-entered here explicitly in a later state update while keeping shadow strategies local-only during Stage 2 and outside the approved IBKR PaperTrader lane.
